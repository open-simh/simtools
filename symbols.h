 ...
 *     "ch" must be exactly one of "ABCDEFGHIJKLMNOPQRSTUVWXYZ$.0123456789" */

#define DEBUG_SYM_NAME(ch) "$M11." ch ""      /* "Prefix" + <one radix50 char> + "suffix" */
#define DEBUG_SYM_DLEVEL DEBUG_SYM_NAME("D")  /* Debug level (>0) */
#define DEBUG_SYM_ERRCNT DEBUG_SYM_NAME("E")  /* Reported error count (>=0) */
#define DEBUG_SYM_STRICT DEBUG_SYM_NAME("S")  /* Strictness (<0=Relaxed, 0=Neither, >0=Strict) */

#define ADD_DEBUG_SYM(sym, val) \
    add_sym(sym, val, \
           (SYMBOLFLAG_PERMANENT | SYMBOLFLAG_DEFINITION), \
            &absolute_section, &symbol_st, NULL)

#define UPD_DEBUG_SYM(sym, val) lookup_sym(sym, &symbol_st)->value = val

/* Function to find out if a symbol is imported */
...
#define SYMBOLFLAG_NONE             0  /* No flags */
...
enum pseudo_ops {
    ...
    P_IFT,
    P_IFTF,
    P_IFXX,  /* .IFxx where xx=DF NDF Z EQ NZ NE L LT G GT LE GE */
    P_IIF,
    P_INCLUDE,
    P_IRP,
    P_IRPC,
...
enum operand_codes { OC_MASK = 0xff00,
    ...
    OC_SOB        = 0x0800,         /* SOB */
    OC_FPP_FSRCAC = 0x0900,         /* FPP (fsrc gen, floating ac 0-3) */
    OC_FPP_SRCAC  = 0x0a00,         /* FPP (src gen, floating ac 0-3) */
    OC_FPP_ACFDST = 0x0b00,         /*  */
    OC_FPP_ACDST  = OC_FPP_ACFDST,  /* FPP (floating ac 0-3, fdst gen) */
 /* OC_FPP_FSRC   = 0x0c00, */      /* FPP fp source: immediate or fsrc gen */
    OC_FPP_FDST   = OC_1GEN,        /* FPP fdst general destination */
    OC_CIS2       = 0x0d00,         /* CIS with 2 parameter words */
    OC_CIS3       = 0x0e00,         /* CIS with 3 parameter words */
    OC_CIS4       = 0x0f00,         /* CIS with 4 parameter words */
    OC__LAST      = 0xff00
};
